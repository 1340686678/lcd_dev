...
int main(void)
{
  ...
  MX_I2C1_Init();
  /* USER CODE BEGIN 2 */
	comm_port_uart1.init_func(&comm_port_uart1);
	comm_port_spi3.init_func(&comm_port_spi3);
	comm_port_i2c1.init_func(&comm_port_i2c1);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOD, LCD_CS_Pin|LCD_RST_Pin|LCD_RS_Pin, GPIO_PIN_SET);
  ...
  /* EXTI interrupt init*/
  HAL_NVIC_SetPriority(TOUCH_IRQ_EXTI_IRQn, 0, 0);
